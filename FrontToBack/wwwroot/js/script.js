$(document).ready(function () {
    ...
    $(document).on("click", "#loadMore", function () {
        fetch(`Product/Load?skip=${skip}`).then(response => response.text())
            .then(data => {
                $("#productList").append(data);
                skip += 8;

                let producsCount = $("#productsCount").val();

                if (skip >= producsCount) {
                    $("#loadMore").remove();
                }
            })
    })
    ...
    $(document).on("click", ".add-to-basket", function (e) {
        ...
        fetch(url).then(response => response.text()).then(data => $(".shop-cart").html(data));
    })
